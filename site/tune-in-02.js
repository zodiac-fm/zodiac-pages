// Tune In 02: one persistent player, real per-song waveform data, real per-Hz audio.
const $ = (id) => document.getElementById(id);
const API = '/api/tune-in-01';
const DOMAINS = [
  { key: 'core', name: 'Core', color: '#6521C9', end: '#A737C8', soft: '#D2C8FF', tint: '#EFECFF', text: '#6521C9' },
  { key: 'love', name: 'Love', color: '#B90044', end: '#CF4090', soft: '#FFBDC3', tint: '#FFE8EA', text: '#B90044' },
  { key: 'vitality', name: 'Vitality', color: '#FF9112', end: '#FFC251', soft: '#FFC392', tint: '#FFEADA', text: '#C25500' },
  { key: 'abundance', name: 'Abundance', color: '#12A56E', end: '#46D99A', soft: '#8FE1BE', tint: '#DCF4E8', text: '#0A7A52' },
];
const TRACKS = [
...
const EXAMPLE = Object.freeze({ core: 422, love: 432, vitality: 579, abundance: 611 });
...
function renderSongs() {
  const freeRow = (item) => `<article class="song-row ${track.id === item.id ? 'is-current' : ''}" data-track="${item.id}"><div class="song-top"><button class="song-play" type="button" data-play-track="${item.id}" aria-label="Play ${item.name}"><span class="song-play-icon">${PLAY}</span><span class="song-name">${item.name}</span></button><span class="song-time">${item.duration}</span></div><button class="wave-seek" type="button" data-seek-track="${item.id}" aria-label="Seek within ${item.name}">${waveformMarkup(item, track.id === item.id ? playbackProgress() : 0)}</button><div class="song-tags">${item.tags}</div><div class="song-meta"><span><i class="dot"></i>${domain.name} · ${hz[domain.key]} Hz</span><span>Free</span></div></article>`;
...
function renderFrequencies() {
  ...
  $('profileBadge').textContent = personalized ? 'YOUR FREQUENCIES' : 'SAMPLE · PRESS PLAY';
  $('stageMeta').textContent = `${domain.name} · ${hz[domain.key]} Hz · ${bandName(hz[domain.key])} band`;
  $('personalizeTitle').textContent = personalized ? 'Tuned to you.' : 'Make it yours.';
  $('personalizeText').textContent = personalized ? 'Your four frequencies are ready.' : 'Hear this music with your own frequencies.';
  if ($('birthSummary')) {
    $('birthSummary').textContent = personalized ? usedNoonEstimate ? 'Using noon as an estimated birth time.' : 'Calculated from your birth details.' : '';
    $('birthSummary').hidden = !personalized;
  }
  const personalizeButton = document.querySelector('.personalize [data-action="personalize"]');
  if (personalizeButton?.firstChild?.nodeType === Node.TEXT_NODE) personalizeButton.firstChild.textContent = personalized ? 'Edit birth details' : 'Add birth details';
  renderSongs();
  renderBands();
  drawFrequency();
}
...
function updatePlayback() {
  ...
  $('stageTitle').textContent = track.name;
  ...
  $('miniToggle').innerHTML = playing ? PAUSE : PLAY;
  $('miniToggle').setAttribute('aria-label', playing ? 'Pause music' : 'Play music');
  ...
  $('tonePlay').lastChild.textContent = toneOn ? 'Pause tone' : 'Hear the tone';
  $('visualCaption').textContent = toneOn
    ? `Reference tone · ${domain.name} · ${hz[domain.key]} Hz`
    : personalized ? `Your ${hz[domain.key]} Hz tone, carried through the music.` : `Sample recording · tone embedded at ${hz[domain.key]} Hz`;
...
function updateMiniPlayer() {
  $('miniPlayer').hidden = !((audioStarted && audio.src) || toneOn) || (stageVisible && !document.querySelector('dialog[open]')) || document.body.classList.contains('player-expanded');
}
...
async function switchDomain(key) {
  ...
  if (wasTone && oscillator && context) {
    ...
    updatePlayback();
  } else if (audio.src || loading) await loadTrack({ at: position, resume: wasPlaying });
}
...
function drawFrequency() {
  ...
  if (toneOn) {
    ...
    for (let i = 0; i <= 320; i++) {
      ...
      const y = target + Math.sin(i * currentHz() / 62) * Math.sin(Math.PI * i / 320) * Math.min(28, height * 0.09);
  ...
  canvas.setAttribute('aria-label', toneOn
    ? `${domain.name} reference tone at ${currentHz()} hertz`
...
$('unknownTime').addEventListener('change', () => {
  ...
  if ($('unknownTime').checked) $('birthTime').value = '12:00';
});
$('birthForm').addEventListener('submit', async (event) => {
  ...
  const clock = $('unknownTime').checked ? '12:00' : $('birthTime').value;
  if (!date || !clock || !$('birthDate').validity.valid) {
    $('birthError').textContent = 'Add a valid birth date and time, or choose “Unknown birth time?”';
    return;
  }
  ...
  try {
    ...
    const result = await api(API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'calculate', year, month, day, hour, min, lat: selectedPlace.latitude, lon: selectedPlace.longitude, tzone }) });
    ...
    else if (hadAudio) await loadTrack({ at: position, resume: wasPlaying });
    closeDialog($('birthDialog'));
    message('Your frequencies are ready.');
  } catch (error) {
    $('birthError').textContent = error.message;
  } finally {
    $('findFrequencies').disabled = false;
    $('findFrequencies').textContent = 'Find my frequencies';
  }
});
...
document.addEventListener('click', async (event) => {
  ...
  switch (target.dataset.action) {
    case 'personalize': openDialog($('birthDialog'), $('birthDate')); break;
...
$('tonePlay').addEventListener('click', playTone);
...
for (const dialog of document.querySelectorAll('dialog')) dialog.addEventListener('close', () => { lastFocus?.focus?.({ preventScroll: true }); updateMiniPlayer(); });
if ('mediaSession' in navigator) {
  for (const [action, handler] of Object.entries({
    play: () => { if (audio.paused) playTrack(track); },
    pause: () => audio.pause(),
    previoustrack: () => playTrack(FREE[(FREE.findIndex((item) => item.id === track.id) + FREE.length - 1) % FREE.length]),
    nexttrack: () => playTrack(FREE[(FREE.findIndex((item) => item.id === track.id) + 1) % FREE.length]),
  })) {
