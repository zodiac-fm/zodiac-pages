...
function songRow(item) {
  ...
  return `<article class="song-row${locked ? ' is-locked' : ''}${selected ? ' is-current' : ''}" data-${locked ? 'locked' : 'track'}="${item.id}" role="button" tabindex="0" aria-label="${locked ? 'Explore membership for' : 'Hear'} ${item.name}"><span class="song-action" aria-hidden="true">${locked ? SVG_LOCK : SVG_PLAY}</span><div class="song-body">${waveformMarkup(item)}<div class="song-copy"><strong class="song-name">${item.name}</strong><span class="song-time">${item.duration}</span></div></div></article>`;
}
