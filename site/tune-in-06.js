// Tune In 06 demo: offline birthplace finder and sample frequencies, one persistent audio element.
...
function setInstruction() {
  $('instruction').textContent = !frequencies ? 'Enter your birth details to try the demo player.'
    : mode === 'music' && currentTrack ? `${currentTrack.name} carries the sample ${domain.name} tone.`
      : heardTone ? 'Hear this sample tone carried through music below.'
        : `Start with the sample ${domain.name} tone, then choose a song.`;
...
for (const id of ['appLink', 'appContinue']) {
  $(id).href = '#';
  $(id).removeAttribute('target');
  $(id).removeAttribute('rel');
}
$('appContinue').addEventListener('click', (event) => {
  event.preventDefault();
  $('membershipTitle').textContent = 'Explore Zodiac';
  $('membershipDialog').showModal();
});
